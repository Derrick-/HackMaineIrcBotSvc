/*
 * $Id: IrcClient.cs 292 2009-03-08 14:23:22Z meebey $
...
namespace Meebey.SmartIrc4net
{
    ...
    public class IrcClient : IrcCommands
    {
        ...
        {
            return (Nickname.Equals(nickname, StringComparison.InvariantCultureIgnoreCase));
        }
            ...
            if (ActiveChannelSyncing || PassiveChannelSyncing) {
                ...
                foreach (Channel channel in _Channels.Values) {
                    if (channel.Key.Length > 0) {
                        if (!_AutoRejoinChannels.ContainsKey(channel.Name)) // -=Derrick=- this check was not initially present.
                ...
                foreach (string channel in _JoinedChannels) {
                    if (!_AutoRejoinChannels.ContainsKey(channel)) // -=Derrick=- this check was not initially present.
        ...
        private void _Event_JOIN(IrcMessageData ircdata)
        {
            ...
            if (ActiveChannelSyncing) {
                ...
                if (IsMe(who)) {
                    ...
                    if (!_Channels.Contains(channelname)) // -=Derrick=- this check was not initially present.
